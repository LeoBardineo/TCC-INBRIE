...
<head>
    ...
    <link rel="stylesheet" href="styles/styleAudioPlayerMusica.css">
    <link rel="stylesheet" href="styles/stylePostMusica.css">
...
<body class="preload" id="preloadcontainer">
    <?php
        ...
        $postID = $_GET['postID'];
        $sql = "SELECT id_usuario, postTitulo, postDescricao, miniaturaDiretorio, tipoMusica, generoMusica FROM posttipomusica WHERE postID = $postID";
        ...
        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_assoc($result)){
                ...
                $tipoMusica = $row['tipoMusica'];
                $generoMusica = $row['generoMusica'];
                $miniaturaDiretorio = $row['miniaturaDiretorio'];
                $miniaturaDiretorio = substr($miniaturaDiretorio, 3);
            }
        }else{
            header('Location: error404.php');
            exit;
    ...
    <div class="publicacao-container">
        ...
        <div class="row-content-container">
            <div class="content-container">
                <div class="header-content-container">
                    <div class="miniatura-content-container">
                        <img class="miniatura-musica" src="<?php echo $miniaturaDiretorio; ?>">
                    </div>
                    <div class="title-content-container">
                    <?php
                    if(isset($_SESSION['usuarioID'])){ 
                if($usuarioLogadoID == $id_usuario){
                        echo "<form method='POST' action='zActionsPHP/excluirPost/excluirMusica.php' class='btn-excluir-post'>
                    ...
                    ?>
                        <p><?php echo $tipoMusica; ?></p>
                        <h1><?php echo $postTitulo; ?></h1>
                        <p><?php echo $postDescricao; ?></p>
                        <p>Gênero: <?php echo $generoMusica; ?></p>
                    </div>
                </div>
                ...
                <div class="musica-content-container">
                    ...
                    <?php
                    $sql = "SELECT conteudoIndice, conteudoTitulo, conteudoDiretorio FROM postconteudomusica WHERE id_musicaPost = $postID ORDER BY conteudoIndice ASC";
                    $result = mysqli_query($conexao, $sql);
                    $i = 0;
                    while($row = mysqli_fetch_assoc($result)){
                        $i++;
                        $conteudoIndice = $row['conteudoIndice'];
                        $conteudoTitulo = $row['conteudoTitulo'];
                        $conteudoDiretorio = $row['conteudoDiretorio'];
                        $conteudoDiretorio = substr($conteudoDiretorio, 3);
                        echo "<tr>";
                        echo "<td>$conteudoIndice</td>";
                        echo "<td>$conteudoTitulo</td>";
                        echo "<td>
                        <div class='audio-player'>
                            ...
                            <audio controls src='$conteudoDiretorio' class='audio-path' data-value='$i' id='track[$i][trackContentPreview]' onended='fimMusica(this)' onloadedmetadata='metadataLoaded(this)' ontimeupdate='audioTimeUpdate(this)' ></audio>
                        </div>
                ...
                <div class="tags-container">
                    <?php
                        $sql = "SELECT tag FROM posttagmusica WHERE id_musicaPost = $postID";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
                            $tag = $row['tag'];
                            echo "
                ...
                <div class="info-post">
                    <?php
                        $sql = "SELECT COUNT(*) AS numLikes FROM postcurtidamusica WHERE id_musicaPost = $postID AND curtidaTipo = 1";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
                            $numLikes = $row['numLikes'];
                        }

                        $sql = "SELECT COUNT(*) AS numDeslikes FROM postcurtidamusica WHERE id_musicaPost = $postID AND curtidaTipo = 0";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
            ...
            <div class="buttons-container">
                <form method="POST" action="zActionsPHP/like/likeMusica.php">
                <input type="hidden" id="usuarioLogadoID" name="usuarioLogadoID" style="display: none;" value="<?php 
        if(isset($_SESSION['usuarioID'])){  echo $usuarioLogadoID; }?>">
                <input type="hidden" id="postID" name="postID" style="display: none;" value="<?php 
        if(isset($_SESSION['usuarioID'])){  echo $postID; }?>">
                <?php
                if(isset($_SESSION['usuarioID'])){ 
                ...
                <form method="POST" action="zActionsPHP/like/deslikeMusica.php">
                <input type="hidden" id="usuarioLogadoID" name="usuarioLogadoID" style="display: none;" value="<?php 
        if(isset($_SESSION['usuarioID'])){  echo $usuarioLogadoID; }?>">
        ...
        if(isset($_SESSION['usuarioID'])){ 
                    ...
                    echo "<h2><a class='link-for-login' href='login.php'>Logue aqui</a> para curtir ou descurtir.</h2>";
                }
                ?>
                </form>
            </div>
            ...
            <?php
                $sql = "SELECT COUNT(*) AS numComment FROM postcomentariomusica WHERE id_musicaPost = $postID";
                $result = mysqli_query($conexao, $sql);
                while($row = mysqli_fetch_assoc($result)){
            ...
            <div class="comments-container">
                ...
                <div class="insert-comment-container">
                    ...
                    <form action="zActionsPHP/postarComentario/postarComentarioMusica.php" method="post" class="send-comment-container">
                    <?php
                    if(isset($_SESSION['usuarioID'])){ 
                ...
                <?php
                    $sql = "SELECT usuario.usuarioFotoPerfil, usuario.usuarioNome, postcomentariomusica.comentarioID, postcomentariomusica.comentarioConteudo, postcomentariomusica.comentarioData FROM usuario, postcomentariomusica WHERE usuario.usuarioID = postcomentariomusica.id_usuario AND postcomentariomusica.id_musicaPost = $postID  ORDER BY comentarioData DESC;";
                    $result = mysqli_query($conexao, $sql);
                    if(mysqli_num_rows($result) > 0){
                        while($row = mysqli_fetch_assoc($result)){
                            ...
                            <div class='sended-comment-container'>
                                ...
                                <form method='get' class='send-comment-container' action='zActionsPHP/excluirComentario/excluirComentarioMusica.php?comentarioID='$comentarioID'&postID='$postID'>
    ...
    <?php
        include('footer.html');
    ?>
    <script src="scripts/audioPlayerMusica.js"></script>
</body>
</html>