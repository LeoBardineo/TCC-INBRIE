...
<head>
    ...
    <link rel="stylesheet" href="styles/stylePostLiteratura.css">
...
<body class="preload" id="preloadcontainer">
    <?php
        ...
        $postID = $_GET['postID'];
        $sql = "SELECT id_usuario, postTitulo, postDescricao, miniaturaDiretorio FROM posttipoliteratura WHERE postID = $postID";
    ...
    <div class="publicacao-container">
        ...
        <div class="row-content-container">
            <div class="content-container">
                <div class="header-content-container">
                    <div class="miniatura-content-container">
                        <img class="miniatura-literatura" src="<?php echo $miniaturaDiretorio; ?>">
                    </div>
                    <div class="title-content-container">
                    <?php
                    if(isset($_SESSION['usuarioID'])){ 
                        if($usuarioLogadoID == $id_usuario){
                            echo "<form method='POST' action='zActionsPHP/excluirPost/excluirLiteratura.php' class='btn-excluir-post'>
                ...
                <div class="lit-content-container">
                    <?php
                        $sql = "SELECT conteudoIndice, conteudo FROM postconteudoliteratura WHERE id_literaturaPost = $postID ORDER BY conteudoIndice ASC";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
                            $conteudoIndice = $row['conteudoIndice'];
                            $conteudo = $row['conteudo'];
                            echo "<div class='literatura-conteudo'>$conteudo</div>";
                        }
                    ?>
                </div>
                <div class="tags-container">
                    <?php
                        $sql = "SELECT tag FROM posttagliteratura WHERE id_literaturaPost = $postID";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
                            $tag = $row['tag'];
                            echo "
                ...
                <div class="info-post">
                    <?php
                        $sql = "SELECT COUNT(*) AS numLikes FROM postcurtidaliteratura WHERE id_literaturaPost = $postID AND curtidaTipo = 1";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
                            $numLikes = $row['numLikes'];
                        }

                        $sql = "SELECT COUNT(*) AS numDeslikes FROM postcurtidaliteratura WHERE id_literaturaPost = $postID AND curtidaTipo = 0";
                        $result = mysqli_query($conexao, $sql);
                        while($row = mysqli_fetch_assoc($result)){
            ...
            <div class="buttons-container">
                <form method="POST" action="zActionsPHP/like/likeLiteratura.php">
        ...
        if(isset($_SESSION['usuarioID'])){ 
                ...
                <form method="POST" action="zActionsPHP/like/deslikeLiteratura.php">
            ...
            <?php
                $sql = "SELECT COUNT(*) AS numComment FROM postcomentarioliteratura WHERE id_literaturaPost = $postID";
                $result = mysqli_query($conexao, $sql);
                while($row = mysqli_fetch_assoc($result)){
            ...
            <div class="comments-container">
                ...
                <div class="insert-comment-container">
                    ...
                    <form action="zActionsPHP/postarComentario/postarComentarioLiteratura.php" method="post" class="send-comment-container">
                    <?php
                    if(isset($_SESSION['usuarioID'])){ 
                ...
                <?php
                    $sql = "SELECT usuario.usuarioFotoPerfil, usuario.usuarioNome, postcomentarioliteratura.comentarioID, postcomentarioliteratura.comentarioConteudo, postcomentarioliteratura.comentarioData FROM usuario, postcomentarioliteratura WHERE usuario.usuarioID = postcomentarioliteratura.id_usuario AND postcomentarioliteratura.id_literaturaPost = $postID  ORDER BY comentarioData DESC;";
                    $result = mysqli_query($conexao, $sql);
                    if(mysqli_num_rows($result) > 0){
                        while($row = mysqli_fetch_assoc($result)){
                            ...
                            <div class='sended-comment-container'>
                                ...
                                <form method='get' class='send-comment-container' action='zActionsPHP/excluirComentario/excluirComentarioLiteratura.php?comentarioID='$comentarioID'&postID='$postID'>
