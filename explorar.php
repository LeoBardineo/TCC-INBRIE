...
<head>
    ...
    <link rel="stylesheet" href="styles/styleExplorar.css">
    ...
    <title>INBRIE - Explorar</title>
</head>
<body class="preload" id="preloadcontainer">
    <?php
        include('header.php');
        if(isset($_GET['organizeExplore']) && isset($_GET['showExplore'])){
    ...
    <div class="container-explorar">
        <div class="title-container-explorar">
            <h1>Explorar</h1>
        </div>
        <div class="separador-horizontal"></div>
        <div class="options-explorar">
        <form class="options-explorar" action="explorar.php" method="GET">
            ...
            <?php
                ...
                if($organizeExplore == "MaisRecentes" || $organizeExplore == "MenosRecentes"){
                    $sql = "SELECT postID, postTitulo, postDescricao FROM $tableTipo $orderBY;";
                }else{
                    $sql = "SELECT postID, postTitulo, postDescricao, COUNT(curtidaTipo) AS numCurtida FROM $tableTipo, $tableCurtida WHERE $tableCurtida.$id_post = $tableTipo.postID GROUP BY $tableTipo.postID ORDER BY numCurtida $orderBY";
                }
                $result = mysqli_query($conexao, $sql);
                if(mysqli_num_rows($result) > 0){
                    echo "
                    <div class='content-container-explorar'>";
                    while($row = mysqli_fetch_assoc($result)){
                        ...
                        if($showExplore == "fotografia" || $showExplore == "ilustracao"){
                            ...
                            echo "
                                <div class='preview-post-explorar' style='background-image:url(".$conteudoDiretorio.")'></div>
                            ";
