    ...
    if(mysqli_num_rows($result) > 0){// o usuário já realizou alguma ação sobre o post
        while($row = mysqli_fetch_array($result)){
            ...
            if($curtidaTipo == 1){ // se ele já curtiu
                ...
                $sql = "UPDATE postcurtidamusica SET curtidaTipo = 1 WHERE id_musicaPost = $postID AND id_usuario = $usuarioLogadoID";
                $result = mysqli_query($conexao, $sql);
                if(!$result){
                    echo mysqli_error($conexao);
                }
        ...
        // inserir curtir
        $sql = "INSERT INTO postcurtidamusica (id_musicaPost, id_usuario, curtidaTipo) VALUES ($postID, $usuarioLogadoID, 1)";
        $result = mysqli_query($conexao, $sql);
        if(!$result){
            echo mysqli_error($conexao);
        }
