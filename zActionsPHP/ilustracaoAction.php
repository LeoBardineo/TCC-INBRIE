<?php
    ...
    if(!$_POST['titleinput'] || !$_POST['descinput']){
        ...
        header('Location: ../postarIlustracao.php');
        exit();
    }
    ...
    $sql = "INSERT INTO posttipoilustracao (id_usuario, postTitulo, postDescricao) VALUES ('$userID', '$postTitulo', '$postDesc')";
    ...
    $sql = "SELECT * FROM posttipoilustracao ORDER BY postID DESC LIMIT 1;";
    $result = mysqli_query($conexao, $sql);
    ...
    if(!is_dir("../arquivos/ilustracao/$lastID")){
        mkdir("../arquivos/ilustracao/$lastID");
    }
    ...
    foreach($nomeImagens as $index => $value){
        ...
        $imagemCaminho = "../arquivos/ilustracao/$lastID/$nomeImagemNova";
        $nomeTemp = $nomeTempImagens[$index]['imageContent'];
        move_uploaded_file($nomeTemp, $imagemCaminho);

        // Inserir
        $sql = "INSERT INTO postconteudoilustracao (id_ilustracaoPost, conteudoIndice, conteudoDiretorio) VALUES ('$lastID', '$index', '$imagemCaminho')";
        $result = mysqli_query($conexao, $sql);
        if(!$result){
    ...
    foreach ($postTagsArray as $index => $value) {
        $sql = "INSERT INTO posttagilustracao (id_ilustracaoPost, tag) VALUES ('$lastID', '$value')";
        $result = mysqli_query($conexao, $sql);
        if(!$result){
            echo "deu errado: ".mysqli_error($conexao);
            exit();
    ...
    $sql = "INSERT INTO posttagilustracao (id_ilustracaoPost, tag) VALUES ('$lastID', '$postTitulo')";
    $result = mysqli_query($conexao, $sql);
    if(!$result){
        echo "deu errado: ".mysqli_error($conexao);
        exit();
